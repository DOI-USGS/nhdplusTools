...
plot_nhdplus <- function(sites, bbox = NA, streamorder = NA, gpkg = NA) {
  ...
  prettymapr::prettymap({
    ...
    plot(gt(pd$basin), lwd = 1, col = NA, border = "black", add = TRUE)
    plot(gt(pd$flowline), lwd = 1, col = "blue", add = TRUE)
    plot(gt(pd$sites), col = "grey40", pch = 17, add = TRUE)
  },
  drawarrow = TRUE)
}
...
get_plot_data <- function(sites, bbox = NA, streamorder = 3, gpkg = NA) {
  ...
  plot_bbox <- sp_bbox(st_transform(basin, 4326))
