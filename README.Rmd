...
`nhdplusTools` is being renamed to `hydrogfetch` (hydrologic geospatial fabric extraction tool chain). The package's scope has grown beyond NHDPlus, and the name is no longer accurate. `hydrogfetch` will be a pure rename. Functions, function signatures, behavior, will all stay the same. `hydrogfetch` will be published to CRAN as a new package.

Development on `nhdplusTools` has stopped; only critical bug fixes will be applied. Once `hydrogfetch` v1.0 is released, `nhdplusTools` will become a deprecation shim that forwards calls to `hydrogfetch` with a deprecation warnings. The shim will remain on CRAN until **October 2028**, at which point `nhdplusTools` will be archived. Archived packages stay installable from the CRAN archive, so code pinned to `nhdplusTools` will continue to run.
