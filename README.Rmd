...
`hydrogeofetch` provides easy access to data associated with the U.S. National Hydrography Dataset. Many functions are thin wrappers around functions that have been migrated to `hydroloom`.

## Package Vision

The `hydrogeofetch` package is intended to provide a reusable set of tools to
...
points, lines, and polygons. `hydrogeofetch` supports both
...
**Release candidates** are built on code.usgs.gov. Push a branch named `rc/<version>` (e.g. `rc/1.0.0`) to trigger the GitLab CI pipeline, which runs three stages:
