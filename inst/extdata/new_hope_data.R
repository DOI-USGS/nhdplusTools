...
if(!file.exists(file.path(data_dir, "new_hope.gpkg"))) {
  zip::unzip(file.path(extdata, "new_hope.gpkg.zip"),
             files = "new_hope.gpkg", exdir = data_dir)
  zip::unzip(file.path(extdata, "new_hope_wbd.gpkg.zip"),
             files = "new_hope_wbd.gpkg", exdir = data_dir)
}

proj <- "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
new_hope_catchment <- sf::read_sf(file.path(data_dir, "new_hope.gpkg"), "CatchmentSP")
new_hope_catchment <- sf::st_transform(new_hope_catchment, proj)
new_hope_flowline <- sf::read_sf(file.path(data_dir, "new_hope.gpkg"), "NHDFlowline_Network")
...
new_hope_wbd <- sf::read_sf(file.path(data_dir, "new_hope_wbd.gpkg"), "HUC12")
# nolint end
