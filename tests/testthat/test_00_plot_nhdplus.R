...
test_that("tile_cache_dir", {
  ...
  orig_dir <- hydrogeofetch_data_dir()

  hydrogeofetch_data_dir(temp_test)

  dir <- hydrogeofetch:::tile_cache_dir()

  expect_equal(dir, file.path(tempdir(check = TRUE), "tile_cache_dir"))

  hydrogeofetch_data_dir(orig_dir)
})

test_that("test_as_outlets", {
  testthat::skip_on_ci()
  expect_equal(hydrogeofetch:::as_outlets(NULL), NULL)

  o <- list(13293970, 13293971)

  expect_equal(hydrogeofetch:::as_outlets(o),
  ...
  expect_equal(hydrogeofetch:::as_outlets(o), list(list(featureSource = "nwissite", featureID = "USGS-05428500")))
  ...
  o <- c("comid", 13293970)
  expect_equal(hydrogeofetch:::as_outlets(o), list(list(featureSource = "comid", featureID = "13293970")))

  o <- list("comid", 13293970)

  expect_error(hydrogeofetch:::as_outlets(o), regexp = "Error trying to interpret outlet specification.*Expected length 2 character vector or list with optional names: featureSource, featureID.*")
  ...
  expect_equal(hydrogeofetch:::as_outlets(o), list(list(featureSource = "comid",
                                        featureID = "13293970"),
  ...
  expect_equal(hydrogeofetch:::as_outlets(o), list(list(featureSource = "nwissite", featureID = "USGS-05427718"),
  ...
  expect_equal(hydrogeofetch:::as_outlets(o), list(list(featureSource = "comid", featureID = "23735691")))

})

test_that("test_styles", {
  testthat::skip_on_ci()
  st <- hydrogeofetch:::get_styles(NULL)
  ...
  st <- hydrogeofetch:::get_styles(list(basin = c(lwd = 2),
                                       flowline = c(col = "test"),
  ...
  expect_error(hydrogeofetch:::get_styles(list(basin = c(lwc = 2))),
               'Expected one ore more of "lwd", "col", or "border" in basins plot_config, got:lwc')

  expect_error(hydrogeofetch:::get_styles(list(lowline = c(bol = "test"))),
               'Expected one or more of "basin", "flowline", "outlets", "network_wtbd", or "off_network_wtbd" in plot_config, got: lowline')

  expect_error(hydrogeofetch:::get_styles(list(flowline = c(bol = "test"))),
               'Expected one ore more of "lwd" and "col" in flowlines plot_config, got:bol')

  expect_error(hydrogeofetch:::get_styles(list(outlets = list(nwissite = list(dch = "."),
...
test_that("comids", {
  ...
  d <- hydrogeofetch:::get_plot_data(comids, flowline_only = FALSE, nhdplus_data = sample_data)
  expect_true(is(d$catchment, "sf"))
  # expect_true(is(d$basin, "sf"))

  b <- hydrogeofetch:::make_basin(d, "catchment", comids)

  expect_null(b)

  Sys.unsetenv("MAKE_BASIN")

  b <- hydrogeofetch:::make_basin(d, "catchment", comids[1:10])
...
test_that("waterbodies", {
  ...
  d <-  hydrogeofetch:::plot_nhdplus(site, cache_data = FALSE)

  expect_equal(names(d), c("plot_bbox", "outlets", "flowline",
                           "basin", "catchment","network_wtbd",
                           "off_network_wtbd"))
  Sys.sleep(2)
  d <-  hydrogeofetch:::plot_nhdplus(site, flowline_only = FALSE,
                                    cache_data = FALSE)
  ...
  d <- hydrogeofetch:::get_plot_data(bbox = bbox, flowline_only = FALSE)
...
test_that("get_waterbody_outlet", {
  ...
  out <-  hydrogeofetch:::get_wb_outlet(lake_comid, d$flowline)
  ...
  lake_comid <- 14711354
  expect_error(hydrogeofetch:::get_wb_outlet(lake_comid, d$flowline),
