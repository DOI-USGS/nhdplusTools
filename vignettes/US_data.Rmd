...
```
`stage_national_data()` assumes you want to stage data in the same folder as the nhdplus_path database and returns a list of .rds files that can be read with readRDS. The flowlines and catchments are [`sf`](https://r-spatial.github.io/sf/) `data.frame`s and attributes is a plain `data.frame` with the attributes from `flowline`. Note that this introduction uses a small subset of the national seamless database as shown in the plot. 
...
nhdplusTools supports a number of indexing use cases. [See the function index for specifics](https://usgs-r.github.io/nhdplusTools/reference/index.html#indexing-and-network-navigation).
...
For more info about `get_flowline_index()` and other indexing functions, see the article `vignette("point_indexing")` about it or the reference page that describes it.
