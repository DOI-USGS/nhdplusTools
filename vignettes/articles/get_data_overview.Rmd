---
title: "hydrogeofetch Data Access Overview"
author: "dblodgett@usgs.gov"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{hydrogeofetch Data Access Overview}
...
if(local) {
  cache_path <- file.path(hydrogeofetch_data_dir(), "data_v_cache")
} else {
  cache_path <- tempdir()
}
...
Since it's original release in 2019, data access in hydrogeofetch has evolved and grown considerably. This vignette, prepared for a workshop in Summer 2024 shows the diversity of data available with hydrogeofetch in one holistic overview.

The function naming in `hydrogeofetch` related to data access uses three common terms:  
...
The following summarizes specific data access functions available for the datasets that `hydrogeofetch` has supporting functionality for.
...
The Network Linked Data Index plays a key role in discovery `hydrogeofetch`. It provides access to an easy network navigation and basin boundary delineation tool. Behind the scenes, the NLDI is based on the network of the NHDPlusV2 -- so has some special functionality related directly to that dataset. Functions that utilize the NLDI include:
...
In the demo below, we'll choose a stream gage and show how to access data related to it from all the datasets that hydrogeofetch works with as well as use some of the NLDI functionality in question.

For the sake of demonstration, we'll look at the Wolf River at Langlade, WI -- site ID "04074950". 

hydrogeofetch allows us to start from a stream gage to build a subset of NHDPlusV2 data. Below, we'll do just that and plot the results on a default map.

```{r}

demo_dir <- hydrogeofetch_data_dir()
...
The above is the original way hydrogeofetch supported access NHDPlusV2 data. A dedicated web service subset utility is available in `get_nhdplus` -- which is what `subset_nhdplus()` calls behind the scenes.
...
For NHDPlusHR data, which is much denser than NHDPlusV2, `hydrogeofetch` supports downloading four-digit Hydrologic Unit Code staged geodatabases. The function `get_huc()` is useful to discover the code needed here.
...
outdir <- file.path(hydrogeofetch_data_dir(), "hr_access_demo")
...
```
At a lower level, we can use `get_hr_data()` to access particular layers. Here, `rename=TRUE` causes the nhdplushr names to be normalized to `hydrogeofetch` conventions using `align_nhdplus_names()`. 
...
gl_coast <- c(hydroloom::navigate_hydro_network(nhdplushr, 60002700000331, "DM"),
              hydroloom::navigate_hydro_network(nhdplushr, 60002700049157, "DM"))
...
`hydroloom::index_points_to_lines()` and `hydroloom::disambiguate_indexes()` are your friends. 

```{r}

potential_matches <- hydroloom::index_points_to_lines(nhdplushr, 
                                        points = site_feature, 
...
hydroloom::disambiguate_indexes(potential_matches,
...
outdir <- file.path(hydrogeofetch_data_dir(), "nhd_access_demo")
...
We'll wait to plot this up until after we've done some work with 3DHP. As of writing, the 3DHP is more or less the same as the NHD but in a new database schema. The `get_3dhp()` uses a web service to pull data for subsets much the same as the other `get_*` functions. See the [3DHP Data Access](get_3dhp_data.html) article for more on how to work with 3DHP data.
...
sub_3dhp <- hydroloom::st_compatibalize(sub_3dhp, nhd_fline)
...
A brand new, May 2024, feature in hydrogeofetch is access to the geoconnex.us reference feature server. The reference feature server provides easy access to representations of datasets that people use to cross reference other data. `discover_geoconnex_reference()` provides access to a full table of what's available. 
...
wbd_dir <- file.path(hydrogeofetch_data_dir(), "wbd_access_demo")

wbd_out <- download_wbd(wbd_dir)

# zip::unzip in download_wbd() doesn't always work -- fall back to utils::unzip
if(length(wbd_out) == 0) {
  f <- list.files(wbd_dir, pattern = ".zip", full.names = TRUE)
  if(length(f) > 0) utils::unzip(f, exdir = wbd_dir)
  wbd_out <- list.files(wbd_dir, pattern = "\\.gdb$",
                        full.names = TRUE, include.dirs = TRUE)
}

if(length(wbd_out) > 0) {
  sf::st_layers(wbd_out[1])
} else {
  message("WBD .gdb not available -- service may be unavailable.")
}
