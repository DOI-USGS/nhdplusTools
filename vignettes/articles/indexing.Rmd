...
if(local) {
  cache_path <- file.path(hydrogeofetch_data_dir(), "index_v")
} else {
  cache_path <- tempdir()
}
...
`hydrogeofetch` offers a few indexing functions and some supporting functions that are worth being aware of. Most of these functions wrap similar but more general functions from [`hydroloom`](https://doi-usgs.github.io/hydroloom/reference/index.html#indexing-and-linear-referencing). 
...
work_dir <- file.path(hydrogeofetch_data_dir(), "index_vignette")
