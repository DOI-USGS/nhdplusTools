...
if(local) {
  cache_path <- file.path(hydrogeofetch_data_dir(), "hr_v_cache")
} else {
  cache_path <- tempdir()
}
...
Most functions in `hydrogeofetch` work with NHDPlus High Res (HR) the same as they do with medium res. The demo below shows how to get a single four-digit hydrologic unit worth of HR data, index a point to it, subset it, and plot the results. 

_Note: For consistency, identifiers from NHDPlusV2 are used with HiRes data. A future release of hydrogeofetch may alter this behavior._
```{r tldr}
library(hydrogeofetch)
library(sf)

work_dir <- file.path(hydrogeofetch_data_dir(), "hr_v_cache")
...
(start_index <- hydroloom::index_points_to_lines(st_transform(hr$NHDFlowline, 5070),
                                   st_transform(plot_data$outlets, 5070),
                                   search_radius = 200)) # meters albers eq area

ids <- hydroloom::navigate_hydro_network(hr$NHDFlowline, start_index$COMID, "UT")
...
The second part of hydrogeofetch support for NHDPlusHR is a bit more involved. The entry point is a function called `get_nhdplushr()`. At it's core, it will take a collection of NHDPlusHR subsets, open them up one by one and build a single output table for later use. It also 
1. joins flowline attributes to the flowline geometry,
2. can return one or more selected layers,
3. can save the result to a standalone geopackage for later use, and
4. can make some attributes of a subset look like they are part of a standalone network rather than having references to downstream data that aren't in the subset.
...
2. `simp` will simplify all geometry to a given tolerance using `sf::st_simplify()`.
3. `proj` will project the output into the desired projection.
4. `keep_cols` is used to specify which attributes are desired in the output.
5. `rename` controls whether output is automatically renamed to be compatible with `hydrogeofetch` functions.
