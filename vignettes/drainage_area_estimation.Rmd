...
knitr::kable(summary_df, digits = 1,
  col.names = c("Basin", "Network DA", "HUC12 DA",
    "HUC10 DA", "HUC08 DA", "NHDPlusHR DA",
...
plot_type <- function(da, fl, basin_label,
  ...
  # Standard is fully transparent (clear).
  col30 <- function(col) adjustcolor(col, alpha.f = 0.3)
  type_colors <- c(
    "Standard" = adjustcolor("white", alpha.f = 0),
  ...
  # ggplot2 4.0 doesn't apply scale fill values to legend keys for levels
  # not present in the data, even with drop = FALSE. A dummy layer with
  # one tiny polygon per level forces each key to render with the
  # correct fill. Placed inside the plot extent and covered by the
  # basin underlay and hu12 layer so they're invisible on the map.
  gage_xy <- st_coordinates(gage_pt)
  dummy_levels <- levels(hu12$type_label)
  dummy_polys <- st_sfc(lapply(seq_along(dummy_levels), \(i)
    st_polygon(list(matrix(
      c(gage_xy[1], gage_xy[2], gage_xy[1] + 1, gage_xy[2],
        gage_xy[1] + 1, gage_xy[2] + 1, gage_xy[1], gage_xy[2] + 1,
        gage_xy[1], gage_xy[2]),
      ncol = 2, byrow = TRUE)))), crs = target_crs)
  dummy_hu12 <- st_sf(
    type_label = factor(dummy_levels, levels = dummy_levels),
    geometry = dummy_polys)

  p <- ggplot() |>
    add_topo(basin_poly, fig_ratio = fig_ratio) |>
    add_basin_underlay(basin_poly) +
    geom_sf(data = dummy_hu12, aes(fill = type_label),
  ...
  p +
    ...
    coord_sf(crs = target_crs, expand = FALSE,
      xlim = fig_lims$xlim, ylim = fig_lims$ylim) +
    labs(title = paste(basin_label, "-- HUC12 Types")) +
    map_theme() +
    theme(legend.box = "vertical") +
    guides(
      fill = guide_legend(
        nrow = 1, order = 1,
        override.aes = list(color = "gray50", linewidth = 0.4)))
}

# Per-basin summary table
basin_summary_table <- function(da, basin_name = NULL) {
  vals <- data.frame(
    source = c("Network", "HUC12",
...
::: {.vignette-note}
- Every HUC12 in the French Broad basin is classified as Standard.
- The uniform clear fill confirms that all units link to a downstream
  HUC12 through normal surface drainage.
:::
...
::: {.vignette-note}
- Closed-basin HUC12s (brown) concentrate in the arid western uplands
  of the Llano Estacado, where surface water never reaches the Brazos.
- Multiple-outlet HUC12s (grey) mark units whose drainage splits
  between more than one downstream HUC.
- Frontal HUC12s (green) appear near the Gulf where catchments drain
  directly to the coast rather than to a downstream HUC12.
- Standard HUC12s (clear) dominate the perennial eastern main stem.
:::
...
::: {.vignette-note}
- Two closed-basin HUC12s (brown) appear in the upper prairie-pothole
  headwaters — these are units with no surface outlet to the James.
- The remaining HUC12s are Standard (clear), including the lower
  perennial main stem.
:::
...
::: {.vignette-note}
- The single HUC12 covering Black Earth Creek is Standard type
  (clear fill), consistent with a well-defined humid basin.
:::
...
::: {.vignette-note}
- The Water HUC12 (blue) is Malheur Lake itself.
- Closed-basin HUC12s (brown) line the pluvial-lake margins, where
  surface drainage terminates in playas rather than reaching the lake.
- Multiple-outlet HUC12s (grey) appear on the low-gradient flats of
  the Harney Basin.
- The surrounding Standard HUC12s (clear) supply the Silvies and
...
::: {.vignette-note}
- Every HUC12 in the Purgatoire basin is Standard type (clear fill).
- The boundary-sensitivity story here is about where the WBD places
  the divide between Standard HUC12s, not about closed-basin or
