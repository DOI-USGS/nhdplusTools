...
- `huc12_outlets` accepts either a path to a GPKG or a pre-loaded `sf`
  data.frame of on-network HUC12 pour points. A national CONUS file is
...
- **NLDI** (`findNLDI`): resolves the start feature, navigates the
  upstream network, and retrieves HUC12 pour points. The `huc12pp`
...
plot_boundaries <- function(da, basin_label) {
  ...
  # Separate layers so HUC12 outlets draw UNDER the basin boundaries
  # while the gage point stays on top. Both still appear in one shape
  # legend because the scales below enumerate both labels.
  hu12_pts <- if(!is.null(hu12_pp))
    st_sf(label = "HUC12 outlet", geometry = st_geometry(hu12_pp)) else NULL
  gage_pts <- st_sf(label = "Stream gage / outlet",
  ...
  if(!is.null(hu12_pts))
    p <- p + geom_sf(data = hu12_pts,
      aes(shape = label, size = label, color = label, alpha = label),
  ...
  p <- p +
    ...
    scale_shape_manual(
      ...
      name = NULL) +
    scale_size_manual(
      values = c("Stream gage / outlet" = 5, "HUC12 outlet" = 1),
      name = NULL) +
    scale_alpha_manual(
      values = c("Stream gage / outlet" = 1, "HUC12 outlet" = 0.75),
      name = NULL) +
    scale_color_manual(
